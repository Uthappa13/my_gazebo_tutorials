#!/usr/bin/env python3

import os

from ament_index_python.packages import get_package_share_directory
from launch import LaunchDescription
from launch.conditions import IfCondition
from launch.actions import IncludeLaunchDescription, DeclareLaunchArgument, ExecuteProcess
from launch.launch_description_sources import PythonLaunchDescriptionSource
from launch.substitutions import LaunchConfiguration
from launch.substitutions import PythonExpression
from launch_ros.substitutions import FindPackageShare
from launch.actions import SetEnvironmentVariable
from launch_ros.actions import Node

def generate_launch_description():
    launch_file_dir = os.path.join(get_package_share_directory('turtlebot3_gazebo'), 'launch')
    pkg_gazebo_ros = get_package_share_directory('gazebo_ros')

    use_sim_time = LaunchConfiguration('use_sim_time', default='true')
    x_pose = LaunchConfiguration('x_pose', default='3.0')
    y_pose = LaunchConfiguration('y_pose', default='3.0')

    ros2_bag_start = LaunchConfiguration('ros2_bag_start')

    ros2_bag_start_arg = DeclareLaunchArgument(
        "ros2_bag_start", 
        default_value='False'
    )

    export_turtlebot_waffle = SetEnvironmentVariable(name='TURTLEBOT3_MODEL', value='waffle')

    export_gazebo_model_path = SetEnvironmentVariable(
    name='GAZEBO_MODEL_PATH',
    value='${GAZEBO_MODEL_PATH}:$(ros2 pkg prefix turtlebot3_gazebo)/share/turtlebot3_gazebo/models/'
)


    world = os.path.join(
        get_package_share_directory('walker'),
        'world/walker_world.world'
    )

    gzserver_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_gazebo_ros, 'launch', 'gzserver.launch.py')
        ),
        launch_arguments={'world': world}.items()
    )

    gzclient_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_gazebo_ros, 'launch', 'gzclient.launch.py')
        )
    )

    robot_state_publisher_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(launch_file_dir, 'robot_state_publisher.launch.py')
        ),
        launch_arguments={'use_sim_time': use_sim_time}.items()
    )

    spawn_turtlebot_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(launch_file_dir, 'spawn_turtlebot3.launch.py')
        ),
        launch_arguments={
            'x_pose': x_pose,
            'y_pose': y_pose
        }.items()
    )

    run_ros2bag_conditioned = ExecuteProcess(
        condition=IfCondition(ros2_bag_start),
        cmd=['ros2', 'bag', 'record', '-o', 'walkerbag', '-a', '-x', '\"/camera/(.*)\"'],
        shell=True
    )

    walker_node = Node(
        package="walker",
        executable="walker"
    )

    ld = LaunchDescription(
        [export_turtlebot_waffle, export_gazebo_model_path, ros2_bag_start_arg, run_ros2bag_conditioned, walker_node]
    )

    ld.add_action(gzserver_cmd)
    ld.add_action(gzclient_cmd)
    ld.add_action(robot_state_publisher_cmd)
    ld.add_action(spawn_turtlebot_cmd)

    return ld